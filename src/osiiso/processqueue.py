"""ProcessQueue — process-based task queue for CPU-bound work.

Use :class:`ProcessQueue` when tasks are CPU-bound and must run in separate
processes to bypass the GIL.  Each item spawns a subprocess managed by a
coordinating daemon thread.  The caller receives a
:class:`~osiiso.SyncTaskHandle` per submission and a
:class:`~osiiso.RunSummary` at the end of ``run()``.
"""
...
def _invoke(item: ProcessItem, result_queue: Any) -> None:
    """Top-level worker function executed inside each subprocess.

    Must be a module-level function so it can be pickled by the
    multiprocessing machinery.

    Args:
        item: The :class:`~osiiso.items.ProcessItem` to execute.
        result_queue: A multiprocessing ``Queue`` used to return the result or
            exception to the coordinating thread.
    """
    try:
...
class _Entry:
    ...
    priority: int
    seq: int
    item: ProcessItem = field(compare=False)
...
class ProcessQueue:
    """Process-based task queue for CPU-bound work.
    ...
    terminates all subprocesses immediately.

    Args:
        workers: Fixed number of concurrent subprocesses.  ``None`` lets the
            queue scale automatically up to ``min(32, cpu_count)``.
        ...
        on_exit: On timeout, ``"complete_priority"`` (default) lets
            must-complete tasks finish; ``"cancel"`` terminates everything.
        poll: Seconds between cancellation/timeout polls while waiting on a
            subprocess.
        context: Optional :class:`multiprocessing.context.BaseContext` to
            control the start method (``"fork"``, ``"spawn"``, or
            ``"forkserver"``).
    ...
    Example::
        ...
        with ProcessQueue(workers=4) as q:
            q.submit(cpu_work, data1)
            q.submit(cpu_work, data2)
            summary = q.run()
    """
    ...
    def __init__(
        ...
        *,
        ...
        Raises:
            ValueError: If *size*, *workers*, *timeout*, or *poll* are out of
                range.
        """
        if size < 0:
        ...
        self._auto_limit = max(1, min(32, os.cpu_count() or 1))
    ...
    def active_count(self) -> int:
        """Number of tasks currently executing in subprocesses."""
        with self._lock:
    ...
    def submit(self, fn: Any, /, *args: Any, opts: TaskOptions | None = None, **overrides: Any) -> SyncTaskHandle:
        """Submit a task for process execution and return a blocking handle.

        Args:
            fn: A pickleable sync callable (or coroutine function) to execute.
            *args: Positional arguments forwarded to *fn*.
            opts: Optional base :class:`~osiiso.TaskOptions`.
            **overrides: Field overrides applied on top of *opts*.

        Returns:
            A :class:`~osiiso.SyncTaskHandle` that blocks on
            :meth:`~osiiso.SyncTaskHandle.wait`.

        Raises:
            ~osiiso.ClosedError: If the queue is not accepting tasks.
            TypeError: If *fn* is a bare awaitable.
        """
    ...
    def task(self, opts: TaskOptions | None = None, **overrides: Any) -> Callable[..., Any]:
        ...
        Example::
            ...
            @q.task(retries=3, timeout=30)
            def crunch(data): ...

            handle = crunch(my_data)
        """
        effective = resolve_opts(opts, overrides)

        def decorator(fn: Any) -> _BoundProcessTask:
            return _BoundProcessTask(fn, self, effective)

        return decorator

    def start(self) -> ProcessQueue:
        """Start coordinator threads and reset transient state for a new run.
        ...
        with self._lock:
            ...
            if not self._started:
                self._started = True
            elif self._workers_count is None:
                need_spawn = True
        if need_spawn:
            self._spawn_workers()
        return self
    ...
    def run(self, timeout: float | None = None, *, strict: bool = False, fail_policy: FailPolicy | None = None) -> RunSummary:
        ...
        try:
            ...
            summary = RunSummary.from_results(collected, run_start=run_start, timed_out=timed_out)
    ...
    def shutdown(self, *, force: bool = False) -> None:
        ...
        Args:
            force: If ``True``, terminate all pending and active subprocesses
    ...
    def cancel(self) -> threading.Thread | None:
        ...
        Safe to call from a coordinator thread; in that case a background
        thread performs the forced shutdown to avoid a deadlock.

        Returns:
            A :class:`threading.Thread` if called from inside a coordinator,
            ``None`` otherwise.
        """
    ...
    def _desired_count(self) -> int:
        if self._workers_count is not None:
    ...
    def _spawn_workers(self) -> None:
        with self._lock:
            ...
            while len(self._threads) < target:
                wid = next(self._wids)
                t = threading.Thread(target=self._worker, args=(wid,), name=f"PTQ-Worker-{wid}", daemon=True)
                self._threads[wid] = t
                to_start.append(t)
        for t in to_start:
            t.start()
    ...
    def _execute(self, entry: _Entry) -> None:
        ...
        running = _Running(cancel_event=threading.Event())

        with self._lock:
            self._active[handle.task_id] = running
        ...
        try:
            while True:
                ...
                try:
                    value = self._run_with_controls(item, running)
                except _Cancelled:
                    raise
                except Exception as exc:
                    if not running.cancel_event.is_set() and handle.attempts <= item.opts.retries:
                        handle._mark_retrying()
                        self._fire_retry(handle, exc)
                        if delay > 0:
                            self._interruptible_sleep(delay, running.cancel_event)
    ...
    def _run_with_controls(self, item: ProcessItem, running: _Running) -> Any:
        ...
        for _ in workers:
            self._queue.put(
                _Entry(
                    ...
                    item=ProcessItem(self._noop, (), TaskOptions()),
                    handle=SyncTaskHandle(
...
class _BoundProcessTask:
    """A function bound to a ProcessQueue via ``@q.task()``."""

    __slots__ = ("_fn", "_queue", "_opts")

    def __init__(self, fn: Any, queue: ProcessQueue, opts: TaskOptions) -> None:
        self._fn = fn
        self._queue = queue
        self._opts = opts
